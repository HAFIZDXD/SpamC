...
print(f"𝗖𝗛𝗔𝗧 𝗚𝗪 𝗗𝗨𝗟𝗨")
sleep(3)
os.system(f"xdg-open https://wa.me/6285758862225")
if "linux" in sys.platform.lower():
	os.system(
			"clear"
	)
...
┈┈┈╰╯┈┈╰╯
  ...
  {red} {white}Status {red}: {green}𝙊𝙉𝙇𝙄𝙉𝙀
  {red} {white}IP Kamu{red}: {green}{ip}
  {red} {white}AUTHOR{red}: {white}HafizdXD
  {red} {yellow}ADMIN TIDAK BERTANGGUNG JAWAB ATAS KESALAHAN YANG
  {red} {yellow}DILAKUKAN PENGGUNA!!!
  
  
 {yellow}MENU{red} :
 {white}[{green}☞︎︎︎1☜︎︎︎{white}] {white}𝙼𝚄𝙻𝙰𝙸 𝚂𝙿𝙰𝙼 (✔︎)
 {white}[{green}☞︎︎︎2☜︎︎︎{white}] {white}𝙻𝙰𝙿𝙾𝚁𝙺𝙰𝙽 𝙱𝚄𝙶(☏︎)
 {white}[{green}☞︎︎︎3☜︎︎︎{white}] {white}𝙲𝙰𝚁𝙰 𝙽𝙶𝙴𝙽𝚃𝙾𝙳 𝙾𝙽𝙻𝙸𝙽𝙴
"""

class Main:
	def __init__(self):
		self.sanz = "Alok Hamil"
		self.main()
	...
	def main(self):
		...
		if sanz == "1" or sanz == "01":
			...
			print(f"\n     {red}! {white}𝙼𝚎𝚗𝚐𝚑𝚞𝚋𝚞𝚗𝚐𝚔𝚊𝚗 𝙺𝚎 𝚆𝚑𝚊𝚝𝚜𝚊𝚙𝚙 𝙰𝚍𝚖𝚒𝚗. . . . . . ")
			# Ganti link fb nya v
...
class sxp_wa:
	...
	def main(self):
		...
		self.wa_otp_5(nomor)
		self.wa_otp_6(nomor)
		print(f" {white}[{green}✓{white}] {white}BERHASIL MENGIRIM SPAM")
...
class sxp_call: # Spam Call 1 Otp
	...
	def call(self, no):
		...
		if "Request misscall berhasil" in sanz.text:
			print(f"    {green}✓ {white}BERHASIL MENGIRIM SPAM")
		else:
